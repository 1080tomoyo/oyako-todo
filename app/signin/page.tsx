...
/**
 * ログイン / 新規登録ページ（親ユーザー）
 * - 1画面で「ログイン」と「新規登録」を切り替えられる
 * - Supabase Auth で認証し、profiles テーブルに表示名を upsert してアプリ内表示に使う
 * - 成功後はマイページへ遷移する
 */
export default function LoginPage() {
  ...
  // signup 時のみ使用する表示名（profiles の display_name に保存）
   ...
   * - Auth の user とは別に、アプリ内で使う表示名を profiles に保存しておく
  ...
  const ensureProfile = async () => {
    ...
    const name = displayName || user.user_metadata?.display_name || user.email || 'ママ';
   ...
   * - 念のため profiles を ensure（初回登録漏れや表示名更新の吸収）
   */
    ...
    <main style={{ maxWidth: 400, margin: '40px auto', padding: 16 }}>
      ...
      <Group justify="center" mt="md">
        {mode === 'signup' ? (
          <Text size="sm">
            すでにアカウントをお持ちの方は{' '}
            <Anchor component="button" type="button" onClick={() => setMode('login')}>
              ログインへ
            </Anchor>
          </Text>
        ) : (
